...
\centering{\texttt{ (:school 'math "jasim" :names 2 my-nested-life) }}
...
\\[1em] {\small \textcolor{black!70}{\texttt{(seq-elt (->  my-nested-life
      ...
      \\ \hspace{7.1em} (map-elt "jasim")
      \\ \hspace{5.5em} person-names)
      \\ \hspace{-3em}2)}}}
% \vspace{1em}
\\{\color{white} Bye!}
#+end_src
...
The savings compound. For example, suppose we have a complicated object like:
#+begin_src emacs-lisp
(cl-defstruct person () names) ;; A person can have multiple names

(setq my-nested-life '((:school math #s(hash-table test equal data ("jasim" #s(person nil (Jimmy J Jameson)))))))
;; ≈ JavaScript: {school: {math: {jasim: {names: [Jimmy, J, Jameson] }}}}
#+end_src

Then the deeply-nested access that used to look like
#+begin_src emacs-lisp
...
Becomes
#+begin_src emacs-lisp 
(:school 'math "jasim" :names 2 my-nested-life) ;; 😍 NEW Syntax
#+end_src

Aside: ~my-nested-life~ was actually constructed using ~(alist :school (list 'math
(hash-map "jasim" (make-person :names '(Jimmy J Jameson)))))~, keep reading to
learn more!
